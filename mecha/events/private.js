...
function ajax_update_set(eid){
	$.ajax({
		...
		success: function(){
			$('#privatecalendar').fullCalendar('removeEvents');
			$('#privatecalendar').fullCalendar('addEventSource', JSON.parse(ajax_getevents()));
			$('#privatecalendar').fullCalendar('rerenderEvents');
			flush_dialog();
		}
		})
}
